...
formatear_fecha<-function (base_datos){
  ...
  mutate(Fecha = paste0(Mes,"/", Year))%>%
  select(Fecha, Year, Mes, Month, everything())
...
formatear_nombres<-function(base_datos,estaciones){
  #estaciones debe contener $Codigo_nacional y bade_datos contiene dos primeras columnas sin nombres
names(base_datos)[-c(1,2)] <- estaciones$Codigo_nacional[match(names(pp_DMC_1989_2018)[-c(1,2)], estaciones$Estacion)]
}
