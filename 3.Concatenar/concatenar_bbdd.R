...
#### ggplot
df_summarized <- result %>%
  mutate(mean_column = rowMeans(select(., 5:890), na.rm = TRUE)) %>%
  select(Year, mean_column) %>%
  group_by(Year) ¿

df_summarized$Codigo_nacional<-as.numeric(df_summarized$Codigo_nacional)
df_summarized <-  df_summarized %>%
  summarize(Codigo_nacional = sum(Codigo_nacional))

ggplot(df_summarized, aes(x = Fecha, y = Codigo_nacional)) +
  geom_line() +
  labs(x = "Fecha", y = "Codigo_nacional") +
  theme_minimal()
